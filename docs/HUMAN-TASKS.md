...
- [ ] AI シートの入力欄を押してキーボードが出たとき、**入力欄がキーボードの上に見えていること**。閉じたあとに画面が上へずれたまま戻らないことがないか（iOS は `position: fixed` をキーボードで動かさず、画面全体をパンして入力欄を見せる。ずれるなら `visualViewport` の `resize` でシートの高さを合わせる実装が要る。Chromium では再現できない）
