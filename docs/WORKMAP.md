...
**Status:** DONE（副表示として解決。既定の表示はそのまま）
**Priority:** P2
**Depends on:** READ-005

### 問題

「PDFは文字選択のUXが悪く、気持ち悪い」。表示・ハイライト・AIへの受け渡しは動くが、選択操作そのものが快適でない。

### なぜそうなるか

pdf.js の text layer は**描画命令ごとの透明なspan**で、並び順はPDFのcontent streamの順序（読む順序とは限らない）、矩形は1行を覆い、語や段落の構造が無い。PDFの性質であって、その表示の上では直せない。

### 解決

紙を描く表示（`Pages`）はそのままに、本文を取り出して流し込む表示（`Text`）を足した。選択・検索・文字サイズが通常のWebページと同じになる。代わりに図・表・段組みのレイアウトを失うので、**どちらが良いかは使い心地で決める**。既定は `Pages`。

ハイライト・読書位置・AIへの受け渡しは両方の表示で共有される。同じデータの2つの見せ方であって、2つの機能ではない。

### Verify

- `tests/unit/pdf-paragraphs.test.mts` — 行幅・行間・字下げからの段落切り出し、行末ハイフンの結合
- `tests/e2e/pdf.spec.ts` — 表示の切り替えと記憶、テキスト表示で付けた印がページ表示にも出ること、選択が正しいページに保存されること、テキスト表示でもページ番号が追従すること
- mutation: 選択のページを画面中央基準に戻す／ページ判定を中央基準に戻す、でそれぞれ赤

判断理由は `docs/DECISIONS.md` D-42〜D-44。
