...
test("the reader fills a narrow viewport and the notes pane stays out of the way", async ({ page }) => {
...
test("a wide viewport hosts the secondary pane beside the reader", async ({ page }) => {
  ...
  // The pane belongs to a document; the library has nothing to mark.
...
test("the library offers a file picker and a useful empty state", async ({ page }) => {
  await page.goto("/");

  await expect(page.getByLabel("Open PDF or EPUB")).toBeAttached();
  await expect(page.getByText("Open a file")).toBeVisible();

  // A fresh browser context has nothing on this device yet.
  const library = page.getByRole("region", { name: "Library" });
  await expect(library).toContainText("Nothing on this device yet");
});
